...
// ── Top quick-access icons ──
const quickItems = [
  { icon: SparklesIcon,  label: "Inicio",     href: "/recomendador",             bg: "bg-brand-dark"   },
  { icon: QrCodeIcon,    label: "Mi QR",      href: "/recomendador/codes",       bg: "bg-brand-purple" },
  { icon: WalletIcon,    label: "Comisiones", href: "/recomendador/comisiones",  bg: "bg-brand-teal"   },
];

// ── Middle nav group ──
const navItems = [
  { icon: LayoutDashboardIcon, label: "Dashboard",    href: "/recomendador"              },
  { icon: BarChart2Icon,       label: "Estadísticas", href: "/recomendador/stats"        },
  { icon: ClockIcon,           label: "Actividad",    href: "/recomendador/referrals"    },
  { icon: MapPinIcon,          label: "Negocios",     href: "/recomendador/businesses"   },
  { icon: UserIcon,            label: "Perfil",       href: "/recomendador/perfil"       },
];

// Bottom nav items (mobile)
const bottomNavItems = [
  { icon: LayoutDashboardIcon, label: "Inicio",     href: "/recomendador"             },
  { icon: ClockIcon,           label: "Actividad",  href: "/recomendador/referrals"   },
  { icon: WalletIcon,          label: "Comisiones", href: "/recomendador/comisiones"  },
  { icon: MapPinIcon,          label: "Negocios",   href: "/recomendador/businesses"  },
  { icon: UserIcon,            label: "Perfil",     href: "/recomendador/perfil"      },
];
...
export function AppSidebar() {
  ...
  const activeNav = navItems.find((item) =>
    item.href === "/recomendador"
      ? pathname === "/recomendador"
        ...
        {quickItems.map((item) => (
          <Tooltip key={item.href}>
            <TooltipTrigger>
              <Link
                href={item.href}
                className="flex h-[66px] w-20 items-center justify-center"
              >
                <div
                  className={`flex h-10 w-10 items-center justify-center rounded-[14px] text-white transition-transform hover:scale-105 active:scale-95 ${item.bg}`}
                >
      ...
      <div className="mb-4 mt-3 flex flex-col items-center">
        <div className="flex flex-col items-center rounded-[15px] bg-[#f8f9fe] p-[3px]">
          <Tooltip>
            <TooltipTrigger>
              <Link
                href="/recomendador/messages"
                className={`flex items-center justify-center rounded-[13px] p-[13px] transition-all ${
                  pathname === "/recomendador/messages"
  ...
  return (
    <>
      ...
      {/* ── Mobile: hamburger trigger button (shown in header area via portal-like absolute) ── */}
      <button
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 flex items-center justify-around bg-white border-t border-border px-2 pb-safe">
        {bottomNavItems.map((item) => {
          const isActive = item.href === "/recomendador"
            ? pathname === "/recomendador"
            : pathname.startsWith(item.href);
          return (
            <Link
              key={item.href}
              href={item.href}
              className="flex flex-col items-center gap-1 py-2.5 px-3 min-w-0"
            >
              <item.icon
